...
const View = ({addResponseFromHome, deleteResponseFromCategory, setDeleteResponseFromView}) => {
  ...
  const [deleteVideoResonseFromVideoCard, setDeleteVideoResonseFromVideoCard] =useState("")
  const [allVideos, setAllVideos] = useState([])

  useEffect(() => {
    getAllVideos()
  }, [addResponseFromHome,deleteResponseFromCategory,deleteVideoResonseFromVideoCard])
  ...
  const categoryVideoDragOverView = async (e)=>{
    ...
    const {video, categoryDetails} = JSON.parse(e.dataTransfer.getData("dargData"))
          ...
          allVideos?.length > 0 ?
            allVideos.map(video => (
              <Col key={video?.id} sm={12} md={6} lg={4}>
                <VideoCard setDeleteVideoResonseFromVideoCard = {setDeleteVideoResonseFromVideoCard} displayData={video}/>
              </Col>
            ))
            : <div className='fw-bolder text-danger fs-5'>No videos Uploaded!!!</div>
        }
      </Row>
    </>
  )
