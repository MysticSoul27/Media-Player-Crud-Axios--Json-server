...
const VideoCard = ({displayData, setDeleteVideoResonseFromVideoCard, insideCategory}) => {
  ...
  const handleShow = async () => {
    ...
    console.log(sysDateTime.toLocaleString('en-US',{timeZone:'Asia/Kolkata'}));
    const timeStamp = sysDateTime.toLocaleString('en-US',{timeZone:'Asia/Kolkata'})
  ...
  const deleteVideo = async (id)=>{
    try{
       ...
       setDeleteVideoResonseFromVideoCard(result)
    }catch(err){
      console.log(err);
  ...
  const videoCardDragStarted = (e, dragVideoDetails) => {
    console.log("Dragged video Details: "+ dragVideoDetails?.id);
    //share the data using event start
  ...
  return (
    <>
      <Card draggable={true} onDragStart={e=>videoCardDragStarted(e,displayData)} style={{ width: '13rem' }} className='mb-2'>
